...
|	ID LPAREN exprList RPAREN	{ $$ = A_CallExp(EM_tokPos, S_Symbol($1), $3); }
...
|	ID LBRACK expr RBRACK OF expr	{ $$ = A_ArrayExp($3->pos, S_Symbol($1), $3, $6); }
...
|	FOR ID ASSIGN expr TO expr DO expr	{ $$ = A_ForExp($4->pos, S_Symbol($2), $4, $6, $8); } 
...
|	ID LBRACK expr RBRACK	{ $$ = A_SubscriptVar(EM_tokPos, A_SimpleVar(EM_tokPos, S_Symbol($1)), $3); }
...
|	VAR ID COLON ID ASSIGN expr	{ $$ = A_VarDec($6->pos, S_Symbol($2), S_Symbol($4), $6); }

funcDecl
:	FUNCTION ID LPAREN typeFields RPAREN EQ expr	{ $$ = A_Fundec($7->pos, S_Symbol($2), $4, NULL, $7); }
|	FUNCTION ID LPAREN RPAREN EQ expr	{ $$ = A_Fundec($6->pos, S_Symbol($2), NULL, NULL, $6); }
|	FUNCTION ID LPAREN typeFields RPAREN COLON ID EQ expr	{ $$ = A_Fundec($9->pos, S_Symbol($2), $4, S_Symbol($7), $9); }
|	FUNCTION ID LPAREN RPAREN COLON ID EQ expr	{ $$ = A_Fundec($8->pos, S_Symbol($2), NULL, S_Symbol($6), $8); }
