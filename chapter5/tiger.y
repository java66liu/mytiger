...
|	expr DIVid2E expr	{ $$ = A_OpExp($1->pos, A_divideOp, $1, $3); }
...
|	id2 LPAREN exprList RPAREN	{ $$ = A_CallExp(EM_tokPos, $1, $3); }
|	id2 LPAREN RPAREN	{ $$ = A_CallExp(EM_tokPos, $1, NULL); }

|	LPAREN exprSeq RPAREN	{ $$ = A_SeqExp($2->head->pos, $2); }
|	LPAREN RPAREN	{ $$ = A_SeqExp(EM_tokPos - 2, NULL); }

|	id2 LBRACE fieldList RBRACE	{ $$ = A_RecordExp(EM_tokPos, $1, $3); }
|	id2 LBRACE RBRACE	{ $$ = A_RecordExp(EM_tokPos, $1, NULL); }

|	id2 LBRACK expr RBRACK OF expr	{ $$ = A_ArrayExp($3->pos, $1, $3, $6); }
...
|	FOR id2 ASSIGN expr TO expr DO expr	{ $$ = A_ForExp($4->pos, $2, $4, $6, $8); } 
...
lvalue
:	id2	{ $$ = A_SimpleVar(EM_tokPos, $1); }
|	lvalue LBRACK expr RBRACK	{ $$ = A_SubscriptVar(EM_tokPos, $1, $3); }
|	id2 LBRACK expr RBRACK	{ $$ = A_SubscriptVar(EM_tokPos, A_SimpleVar(EM_tokPos, $1), $3); }
|	lvalue DOT id2	{ $$ = A_FieldVar(EM_tokPos, $1, $3); }
...
typeDecl
:	TYPE id2 EQ type	{ $$ = A_Namety($2, $4); }

type
:	id2	{ $$ = A_NameTy(EM_tokPos, $1); }
|	LBRACE typeFields RBRACE	{ $$ = A_RecordTy(EM_tokPos, $2); }
|	LBRACE RBRACE	{ $$ = A_RecordTy(EM_tokPos, NULL); }
|	ARRAY OF id2	{ $$ = A_ArrayTy(EM_tokPos, $3); }

typeFields
:	id2 COLON id2	{ $$ = A_FieldList(A_Field(EM_tokPos, $1, $3), NULL); }
|	typeFields COMMA id2 COLON id2	{
						A_fieldList ptr;
						ptr = $1;
						while (ptr->tail != NULL) { ptr = ptr->tail; }
						ptr->tail = A_FieldList(A_Field(EM_tokPos, $3, $5), NULL);
						$$ = $1;
					}

varDecl
:	VAR id2 ASSIGN expr	{ $$ = A_VarDec($4->pos, $2, NULL, $4); }
|	VAR id2 COLON id2 ASSIGN expr	{ $$ = A_VarDec($6->pos, $2, $4, $6); }

funcDecl
:	FUNCTION id2 LPAREN typeFields RPAREN EQ expr	{ $$ = A_Fundec($7->pos, $2, $4, NULL, $7); }
|	FUNCTION id2 LPAREN RPAREN EQ expr	{ $$ = A_Fundec($6->pos, $2, NULL, NULL, $6); }
|	FUNCTION id2 LPAREN typeFields RPAREN COLON id2 EQ expr	{ $$ = A_Fundec($9->pos, $2, $4, $7, $9); }
|	FUNCTION id2 LPAREN RPAREN COLON id2 EQ expr	{ $$ = A_Fundec($8->pos, $2, NULL, $6, $8); }
